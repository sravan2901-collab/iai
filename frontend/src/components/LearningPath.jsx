...
const formatPathDataWithMilestones = (raw) => {
  ...
  const rawLessons = path.path_lessons || [];

  const lessons = rawLessons.map((les, i) => {
    let skillType = les.skill_type || les.skill || (les.category_id === 2 ? 'WRITTEN' : (les.category_id === 3 ? 'READING' : 'SPOKEN'));
    let cType = les.content_type || (skillType === 'WRITTEN' ? 'Written Practice' : (skillType === 'READING' ? 'Functional Reading' : 'Voice Practice'));
    
    // Infer skill type from title or content_type if needed
    const lowerTitle = (les.title || '').toLowerCase();
    if (lowerTitle.includes('written') || lowerTitle.includes('लेखन') || lowerTitle.includes('లేఖన') || lowerTitle.includes('எழுத்து')) {
      skillType = 'WRITTEN';
      cType = 'Written Practice';
    } else if (lowerTitle.includes('reading') || lowerTitle.includes('पठन') || lowerTitle.includes('పఠన') || lowerTitle.includes('வாசிப்பு')) {
      skillType = 'READING';
      cType = 'Functional Reading';
    }
    ...
    return {
      ...
      content_type: cType,
      category_id: skillType === 'WRITTEN' ? 2 : (skillType === 'READING' ? 3 : 1)
    };
  });

  if (lessons.length > 0 && (!path.milestones || path.milestones.length === 0)) {
    const milestone1Lessons = lessons.slice(0, 3);
    const milestone2Lessons = lessons.slice(3, 6);
    const milestone3Lessons = lessons.slice(6);

    const builtMilestones = [
      {
        step: 1,
        title: `Milestone 1: ${path.current_level || 'FOUNDATIONAL'} Multilingual Foundations (Spoken, Written & Reading)`,
        category: "Zero Level Multi-Skill Pillars",
        status: "UNLOCKED",
    ...
    if (milestone2Lessons.length > 0) {
      builtMilestones.push({
        step: 2,
        title: "Milestone 2: Functional Reading & Handwriting Mastery",
        category: "Everyday Words & Handwriting Formation",
    ...
    if (milestone3Lessons.length > 0) {
      builtMilestones.push({
        step: 3,
        title: "Milestone 3: Advanced Literacy & Sentence Expression",
        category: "Literary Expression & Public Signage",
    ...
    return {
      ...
      personalization_reason: path.personalization_reason || `Generated by AI Model based on diagnostic placement test: Multilingual Spoken, Written, and Reading pillars configured.`,
